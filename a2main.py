...
class Decompressor:
    ...
    def run(self) -> int:
        ...
        qdt_file.load()
        tz, pixels_single = self.decompress(qdt_file.body)
    ...
    @staticmethod
    def decompress(qd_tree: QuadTree) -> Tuple[int, List[List[int]]]:
        """
        Decompress by converting the tree into pixels
        """
        return qd_tree.tree_size(), qd_tree.convert_to_pixels()
...
if __name__ == '__main__':
    ...
    ch = input("Command [c -> Compress | d -> Decompress] : ")
    if ch == "q":
        exit(0)
    elif ch == "c":
        ...
        comp = Compressor(name, int(loss))
        ts = comp.run()
        print("Built a tree with size: " + str(ts))
        print("Saving to file: " + name + ".qdt")
        print("Compression Done")
    elif ch == "d":
        name = input("File Name: ")
        print("Decompressing " + name)
        dcom = Decompressor(name)
        ts = dcom.run()
        print("loaded a tree with size: " + str(ts))
        print("Saving to file: " + name + ".bmp")
