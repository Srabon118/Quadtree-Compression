...
class QuadTreeNodeInternal(QuadTreeNode):
    ...
    def tree_size(self) -> int:
        ...
        """
        # TODO: complete this method
        size = sum([node.tree_size() for node in self.children])
        return size + 1
    ...
    def convert_to_pixels(self, width: int, height: int) -> List[List[int]]:
        ...
        """
        # TODO: complete this method
        bl = self.children[0].convert_to_pixels(math.floor(width / 2), math.floor(height / 2))
        br = self.children[1].convert_to_pixels(width - math.floor(width / 2), math.floor(height / 2))
        tl = self.children[2].convert_to_pixels(math.floor(width / 2), height - math.floor(height / 2))
        tr = self.children[3].convert_to_pixels(width - math.floor(width / 2), height - math.floor(height / 2))

        b = [le + ri for le, ri in zip(bl, br)]
        t = [le + ri for le, ri in zip(tl, tr)]
    ...
    def mirror(self) -> None:
        ...
        # TODO
        try:
            if not self.__getattribute__('root'):
                bl = deepcopy(self.children[0])
                br = deepcopy(self.children[1])
                tl = deepcopy(self.children[2])
                tr = deepcopy(self.children[3])
                ...
                for i in range(4):
                    if isinstance(self.children[i], QuadTreeNodeInternal):
                        self.children[i].__setattr__('root', False)
                        self.children[i].mirror()

        except AttributeError:
            self.children[2] = deepcopy(self.children[0])
            self.children[3] = deepcopy(self.children[1])

            self.children[2].__setattr__('root', False)
            self.children[2].mirror()
            self.children[3].__setattr__('root', False)
            self.children[3].mirror()
...
class QuadTree:
    ...
    def _build_tree_helper(self, pixels: List[List[int]]) -> QuadTreeNode:
        ...
        # TODO: complete this method
        if pixels in ([], [[]], [[[]]], [[[[]]]]):
            root = QuadTreeNodeEmpty()
            return root

        stddev, mean = standard_deviation_and_mean(pixels)

        if stddev <= self.loss_level:
            root = QuadTreeNodeLeaf(round(mean))
        else:
            quads = self._split_quadrants(pixels)
            root = QuadTreeNodeInternal()

            for i in range(4):
                root.children[i] = self._build_tree_helper(quads[i])
    ...
    def _split_quadrants(pixels: List[List[int]]) -> List[List[List[int]]]:
        ...
        bl = [pixels[i][:math.floor(w / 2)] for i in range(math.floor(h / 2))]
        br = [pixels[i][math.floor(w / 2):] for i in range(math.floor(h / 2))]
        tl = [pixels[i][:math.floor(w / 2)] for i in range(math.floor(h / 2), h)]
        tr = [pixels[i][math.floor(w / 2):] for i in range(math.floor(h / 2), h)]
...
def maximum_loss(original: QuadTreeNode, compressed: QuadTreeNode) -> float:
    ...
    assert original.tree_size() >= compressed.tree_size()
    loss = float('inf')

    for com, ori in zip(compressed.children, original.children):
        if isinstance(com, QuadTreeNodeInternal):
            loss = min(maximum_loss(ori, com), loss)
        elif isinstance(com, QuadTreeNodeLeaf):
            x = ori.tree_size() - com.tree_size()
            if x != 0:
                s, _ = standard_deviation_and_mean(ori.convert_to_pixels(x, x))
                loss = min(s, loss)
